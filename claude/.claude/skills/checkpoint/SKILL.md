...
1. **Plan-driven session** (plan:* skills used, or a plan doc is loaded): update the plan
   doc: mark completed phases, note current position and next step. Do NOT restructure the
   doc: this is a position marker, not a rewrite. Suggest `/plan:review <slug>` if the doc
   has drifted from what was actually implemented.
