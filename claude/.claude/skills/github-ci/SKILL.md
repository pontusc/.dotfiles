...
- **Pin actions by SHA**, not tag: `uses: actions/checkout@<full-sha>`. Prevents supply-chain attacks from tag mutation. SHAs go stale, so let Dependabot or Renovate (`github-actions` ecosystem) keep them current.
