...
```bash
cmd="omp --model openai-codex/gpt-5.6-luna '$retro retros/<file>.md'"
tmux new-session -d -s retros -n <slug> -c ~/dotfiles "$cmd" 2>/dev/null \
  || tmux new-window -t retros -n <slug> -c ~/dotfiles "$cmd"
```

The initial CLI prompt is processed like typed input, so `$retro` invokes the
retro skill and the doc path routes it to the retro-agent instructions.
