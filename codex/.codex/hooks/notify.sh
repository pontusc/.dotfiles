#!/usr/bin/env bash
set -euo pipefail
# Codex hook relay. Flags the tmux window hosting the Codex pane through the shared
# @claude option consumed by the existing tmux status and workspace tooling.
...
STATE_DIR="${XDG_RUNTIME_DIR:-/tmp}/codex-notify"
...
case "$EVENT" in
  ...
  PostToolUse)
    ...
    ;;
  PermissionRequest)
    raise_flag "ask" "Codex asks"
    ;;
  Stop)
    if [[ ! -f "$STATE_FILE" ]] || (( $(date +%s) - $(<"$STATE_FILE") >= DONE_AFTER )); then
      raise_flag "done" "Codex finished"
    fi
    ;;
esac
